...
public class DBTablesGenerator  extends AbstractDBModelGenerator {
	...
	private String getClassDefinition(DBTable table){
		...
		//Class definition 
		sb.append(PUBLIC + SPACE + CLASS + SPACE + newClassName + SPACE + EXTENDS + SPACE + getClassName(DBTable.class)  + SPACE + BRACKET_BEGIN + SPACE + NEW_LINE);
		sb.append(NEW_LINE + NEW_LINE);
		sb.append("\t" + PUBLIC + SPACE + STATIC + SPACE + FINAL + SPACE + STRING + SPACE + TABLE_NAME_FIELD + SPACE + VAR_AFFECT + SPACE + "\""+table.getName()+ "\"" + END_STATEMENT_LINE) ;
